...
**Company lens (independent):** Baseten (`baseten`)  
**Innovation:** Serving SLO Circuit  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Baseten affiliation, proprietary access, or production deployment.
...
Track finite SLO metrics and trip a circuit breaker with explicit recovery gates before re-admission.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Baseten employment, endorsement, or proprietary systems access.
...
File: `src/serving_slo_circuit.py`

- Expand `ServingSloCircuit` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_serving_slo_circuit.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_serving_slo_circuit.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Baseten APIs
